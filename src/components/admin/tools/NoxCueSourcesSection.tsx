...
function ErrorIncidentsPanel({ source }: { source: NoxCueSource }) {
  ...
  return <Panel>
    <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><AlertTriangle size={16} /><h3 className="text-sm font-semibold text-stone-900">Error incidents</h3></div><p className="mt-1 text-xs leading-5 text-stone-500">Similar failures are grouped into one incident with occurrence and affected-user counts. Resolving is explicit; a new occurrence reopens the group.</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${unresolved.length ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}>{unresolved.length ? `${unresolved.length} unresolved` : "No action required"}</span></div>
    {metrics.isLoading ? <Spinner className="h-4 w-4 text-accent" /> : metrics.isError ? <p className="text-xs text-red-600">Could not load error incidents.</p> : unresolved.length ? <div className="space-y-3">{unresolved.map((group) => <article key={group.fingerprint} className={`rounded-lg border p-4 ${group.status === "open" ? "border-red-200 bg-red-50/60" : "border-amber-200 bg-amber-50/60"}`}>
      <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className={`text-[10px] font-semibold uppercase tracking-[0.16em] ${group.status === "open" ? "text-red-600" : "text-amber-700"}`}>{group.status === "open" ? "Action required" : "Acknowledged"}</p><span className="rounded-full bg-white/80 px-2 py-0.5 text-[10px] font-medium text-stone-500">{group.groupingKind === "explicit" ? "App fingerprint" : "Automatically grouped"}</span></div><p className="mt-1 text-sm font-semibold text-stone-900">{group.title}</p><p className="mt-1 break-all font-mono text-[10px] text-stone-500">{group.fingerprint}</p><p className="mt-2 text-xs text-stone-600"><strong>{group.occurrenceCount}</strong> occurrence{group.occurrenceCount === 1 ? "" : "s"} · <strong>{group.affectedUserCount}</strong> affected user{group.affectedUserCount === 1 ? "" : "s"} · last seen {new Date(group.lastSeenAt).toLocaleString()}</p>{group.firstRelease || group.lastRelease ? <p className="mt-1 text-[11px] text-stone-500">Release {group.firstRelease ?? "unknown"}{group.lastRelease && group.lastRelease !== group.firstRelease ? ` → ${group.lastRelease}` : ""}</p> : null}</div><div className="flex gap-2">{group.status === "open" ? <button type="button" disabled={update.isPending} onClick={() => update.mutate({ fingerprint: group.fingerprint, status: "acknowledged" })} className="rounded-lg border border-amber-200 bg-white px-2.5 py-1.5 text-xs font-medium text-amber-800 disabled:opacity-50">Acknowledge</button> : <button type="button" disabled={update.isPending} onClick={() => update.mutate({ fingerprint: group.fingerprint, status: "open" })} className="rounded-lg border border-stone-200 bg-white px-2.5 py-1.5 text-xs font-medium text-stone-700 disabled:opacity-50">Reopen</button>}<button type="button" disabled={update.isPending} onClick={() => update.mutate({ fingerprint: group.fingerprint, status: "resolved" })} className="rounded-lg bg-stone-900 px-2.5 py-1.5 text-xs font-medium text-white disabled:opacity-50">Resolve</button></div></div>
      {group.sample?.error?.message || group.sample?.message || group.sample?.error?.stack ? <details className="mt-3 rounded-lg border border-stone-200/80 bg-white/70 p-3"><summary className="cursor-pointer text-xs font-medium text-stone-700">Representative sanitized sample</summary>{group.sample.error?.message || group.sample.message ? <p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-stone-700">{group.sample.error?.message ?? group.sample.message}</p> : null}{group.sample.error?.stack ? <pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap rounded bg-stone-950 p-3 text-[10px] leading-4 text-stone-100">{group.sample.error.stack}</pre> : null}</details> : null}
