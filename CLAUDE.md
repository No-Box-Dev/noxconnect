...
Opened and merged narrations store two views from the same generation: the existing first-person `summary` and `events.technical_summary` (migration `0046_event_technical_summary.sql`). The technical view is always exactly three simple-English lines: **What it does**, **How it works**, and **What it touches**. NoxFeed's `service/src/generation.js` owns the provider call, accepts the requested JSON pair, validates and limits output, and deterministically creates the three technical lines when model output is malformed. Merge-time reuse carries both views forward from `pr_narrative`; NoxConnect creates a deterministic fallback only when the product service reports generation unavailable or an older opened row lacks `technical_summary`.
...
The Release-notes prompt is admin-editable in Settings → Release notes prompt (`ReleaseNotesPromptSection`). Empty/missing falls back to the NoxFeed-owned default exposed by the private `NOXFEED_RESPONSE` service binding. Stored under `settings.releaseNotesPrompt` (D1 config). Posts and Release notes use NoxFeed's product-owned model; the organization's `LlmSettingsSection` controls whether managed generation is enabled.
...
**Managed AI (admin-only)** — `LlmSettingsSection` lets an org admin enable or disable server-owned AI and shows separate readiness for NoxFeed generation and NoxConnect matching. The browser never accepts or receives provider credentials, endpoints, or model choices. NoxFeed reports its provider, model, and key availability over the private service binding; NoxConnect reports its own shared managed-AI configuration. Failures fall back to deterministic summaries with diagnostics in `op_failures`.
